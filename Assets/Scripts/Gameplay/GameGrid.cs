...
namespace Match3
{
    public class GameGrid : MonoBehaviour
    {
        ...
        [SerializeField] private float _shiftCell = 0.5f;

        [SerializeField] private PiecePosition[] _initialPieces;
        ...
        private void Awake()
        {
            ...
            {
                if (!_piecePrefabDict.ContainsKey(_piecePrefabs[i].type))
                {
                    _piecePrefabDict.Add(_piecePrefabs[i].type, _piecePrefabs[i].prefab);
                }
            }
            ...
            for (int x = 0; x < xDimension; x++)
            {
                ...
                {
                    GameObject background = Instantiate(_backgroundPrefab, GetWorldPosition(x - _shiftCell, y - _shiftCell), Quaternion.identity);
            ...
            for (int i = 0; i < _initialPieces.Length; i++)
            {
                if (_initialPieces[i].x >= 0 && _initialPieces[i].y < xDimension
                                            && _initialPieces[i].y >=0 && _initialPieces[i].y <yDimension)
                {
        ...
        private bool FillStep()
        {
            ...
            for (int y = yDimension - 2; y >= 0; y--)
            {
                ...
                {
                    int x = loopX;
                    if (_inverse) { x = xDimension - 1 - loopX; }
        ...
        {
            return new Vector2(
                transform.position.x - xDimension / 2.0f + x,
